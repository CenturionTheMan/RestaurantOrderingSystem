...
void GuiNcurses::DrawLeft()
{
    ...
    stream << "-Banana:" << std::setw(4) << std::right << Containers::fridge[PancakeIngredient::Banana] << "/" << Containers::fridgeIngredientLimits[PancakeIngredient::Banana] << std::endl;
    mvwprintw(leftWindow, 4, 2, "%s", stream.str().c_str());
    stream.clear();

    stream << "-Blueberry:" << std::setw(4) << std::right << Containers::fridge[PancakeIngredient::Blueberry] << "/" << Containers::fridgeIngredientLimits[PancakeIngredient::Blueberry] << std::endl;
    mvwprintw(leftWindow, 5, 2, "%s", stream.str().c_str());
    stream.clear();

    stream << "-Chocolate:" << std::setw(4) << std::right << Containers::fridge[PancakeIngredient::Chocolate] << "/" << Containers::fridgeIngredientLimits[PancakeIngredient::Chocolate] << std::endl;
    mvwprintw(leftWindow, 6, 2, "%s", stream.str().c_str());
    stream.clear();

    stream << "-Eggs/Milk/Flour:" << std::setw(4) << std::right << Containers::fridge[PancakeIngredient::Eggs_Milk_Flour] << "/" << Containers::fridgeIngredientLimits[PancakeIngredient::Eggs_Milk_Flour] << std::endl;
