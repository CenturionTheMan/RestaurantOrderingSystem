...
void GuiNcurses::DrawContent()
{
    ...
    mvwvline(contentWindow, 21, 29, (int)'#', 3);
    mvwvline(contentWindow, 21, 40, (int)'#', 3);
    mvwprintw(contentWindow, 21, 30, " GOING TO ");
    mvwprintw(contentWindow, 22, 30, "  BUFFET  ");
    mvwhline(contentWindow, 20, 29, (int)'#', 12);
    mvwhline(contentWindow, 23, 29, (int)'#', 12);
    mvwprintw(contentWindow, 24, 29, "%s", stream.str().c_str());
    stream.str(std::string());
...
void GuiNcurses::RunGui()
{
    ...
    std::thread inputThread([this, &exit](){
        while (!exit)
        {
            ...
            if(tmp != '\n' && this->userInputStream.str().size() < WINDOW_WIDTH * 75/100 - 17)
            {
