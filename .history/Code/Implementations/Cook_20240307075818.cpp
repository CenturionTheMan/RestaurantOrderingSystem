...
void Cook::Run(std::map<PancakeType, int>& buffet, std::mutex& mutex, std::condition_variable& conditionVariable)
{
    std::thread cookThread([this, &buffet, &mutex, &conditionVariable] {
        ...
        while (Cook::shouldAbort == false)
        {
            ...
            conditionVariable.wait(uLock, [&buffet, this] { 
                return buffet[Cook::specializedInPancakeType] < Manager::GetPancakesLimit(PancakeType::BananaPancakes);
            });
            buffet[Cook::specializedInPancakeType]++;
            uLock.unlock();
