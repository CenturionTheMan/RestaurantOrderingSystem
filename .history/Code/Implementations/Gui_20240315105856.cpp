...
void Gui::PrintRestaurant(EntityController& entityController)
{
    ...
    "|  - WaitingForBuffet: " << std::setw(4) << std::right << entityController.GetAmountOfClientsInGivenState(ClientState::ClientGoingToBuffet) << std::endl <<
...
void Gui::PrintUserInput()
{
    ...
    << Gui::COMMAND_DELETE_COOK_CHOCOLATE + "^X" << " | " << Gui::COMMAND_DELETE_COOK_BLUEBERRY + "^X" << " | " << std::endl << BREAK_LINE;
