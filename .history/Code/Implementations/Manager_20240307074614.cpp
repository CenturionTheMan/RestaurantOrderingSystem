...
int Manager::ingredientsAmount = 100;
...
void Manager::CreateAndRunCook(std::map<PancakeType, int>& buffet, std::mutex& mutex, PancakeType cookSpecialization)
{
    ...
    switch (cookSpecialization)
    {
        case PancakeType::BananaPancakes:
            newCook->Run(buffet, mutex, Manager::bananaPancakesLimitCondition);
            bananaCooks.push_back(newCook);
            break;
        case PancakeType::BlueberryPancakes:
            newCook->Run(buffet, mutex, Manager::blueberryPancakesLimitCondition);
            blueberryCooks.push_back(newCook);
            break;
        case PancakeType::ChocolatePancakes:
            newCook->Run(buffet, mutex, Manager::chocolatePancakesLimitCondition);
