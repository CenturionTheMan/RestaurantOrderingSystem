...
{
    int amountToAdd = Containers::fridge[type] + amount > Containers::fridgeIngredientLimits[type] ? Containers::fridgeIngredientLimits[type] - Containers::fridge[type] : amount;
    Containers::fridge[type] += amountToAdd;
}
...
bool Containers::TakePancakeFromBuffet(PancakeType type)
{
    if (Containers::buffet[type] == 0)
    {
        return false;
    }
